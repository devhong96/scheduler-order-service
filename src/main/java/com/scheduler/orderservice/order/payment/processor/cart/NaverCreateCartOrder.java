...
public class NaverCreateCartOrder implements CreateOrderProcessor {
    ...
    public Boolean supports(Vendor vendor, OrderType orderType) {
        return vendor == NAVER && orderType == CART;
    }
    ...
    public void process(OrderType orderType, OrderCategory orderCategory, StudentResponse studentResponse,
        ...
        for (CartDto cartDto : cartDtoByStudentId) {
            ordersList.add(Orders.create(NAVER, CART, orderCategory, studentResponse,
                    cartDto.getProductName(), cartDto.getQuantity(),
                    paymentHistoryDto)
            );
        }
